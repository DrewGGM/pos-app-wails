...
class ServerDiscovery(private val context: Context? = null) {
    ...
    private val waiterPrefs: WaiterPreferences? = context?.let { WaiterPreferences(it) }
     ...
     */
    suspend fun discoverServerWithMode(): ServerConnection? = withTimeoutOrNull(15000) {
    ...
    private suspend fun tryTunnelConnection(): ServerConnection? {
        val tunnelEnabled = waiterPrefs?.tunnelEnabled ?: false
        val tunnelUrl = waiterPrefs?.tunnelUrl
        val useSecure = waiterPrefs?.tunnelUseSecure ?: true
